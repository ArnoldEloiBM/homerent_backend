...
exports.startAdminLandlordConversation = startAdminLandlordConversation;
...
function enforceConversationRules(conv) {
    if (conv.admin_landlord) {
        if (conv.tenant_id != null || conv.property_id != null) {
            throw new http_1.HttpError(500, "Invalid conversation: admin-landlord threads must not set tenant or property");
        }
        return;
    }
    if (conv.tenant_id == null || conv.property_id == null) {
        throw new http_1.HttpError(500, "Invalid conversation: tenant–landlord chat requires tenant and property");
    }
}
function ensureParticipant(conv, user) {
    if (conv.admin_landlord) {
        const landlord = conv.landlord_id === user.id && user.role === "landlord";
        const admin = user.role === "admin";
        if (!landlord && !admin)
            throw new http_1.HttpError(403, "Forbidden in this conversation");
        return;
    }
...
async function listConversations(req, res) {
    ...
    let result;
    if (user.role === "tenant") {
        result = await db_1.db.query(`SELECT c.id, c.tenant_id, c.landlord_id, c.property_id, c.admin_landlord, c.created_at,
       ...
       ORDER BY ${contactBucketOrderSql},
         COALESCE(${lastMsgAtSql}, c.created_at) DESC`, [user.id]);
    }
...
async function sendMessage(req, res) {
    ...
    const convResult = await db_1.db.query("SELECT * FROM conversations WHERE id = $1", [body.conversationId]);
    ...
    if (imageFile && videoFile) {
        throw new http_1.HttpError(400, "Send either an image or a video, not both");
    }
    if (imageFile && !IMG_MIME.test(imageFile.mimetype)) {
        throw new http_1.HttpError(400, "Unsupported image type");
    }
    if (videoFile && !VID_MIME.test(videoFile.mimetype)) {
        throw new http_1.HttpError(400, "Unsupported video type (use MP4, WebM, or MOV)");
    }
    if (!body.content?.trim() && !imageFile && !videoFile) {
        throw new http_1.HttpError(400, "Send either an image, a video, or a text message");
    }
    let imageUrl = null;
    let videoUrl = null;
    if (imageFile) {
        imageUrl = await (0, uploadService_1.uploadBufferToCloudinary)(imageFile.buffer, "homerent/messages", imageFile.mimetype, "image");
    }
    else if (videoFile) {
        videoUrl = await (0, uploadService_1.uploadBufferToCloudinary)(videoFile.buffer, "homerent/messages", videoFile.mimetype, "video");
    }
    const result = await db_1.db.query("INSERT INTO messages (conversation_id, sender_id, content, image_url, video_url) VALUES ($1,$2,$3,$4,$5) RETURNING *", [body.conversationId, req.user.id, body.content?.trim() || null, imageUrl, videoUrl]);
    res.status(201).json(result.rows[0]);
}
async function listMessages(req, res) {
    const conversationId = Number(req.params.conversationId);
    const convResult = await db_1.db.query("SELECT * FROM conversations WHERE id = $1", [conversationId]);
    if (!convResult.rowCount)
        throw new http_1.HttpError(404, "Conversation not found");
    const conv = convResult.rows[0];
    enforceConversationRules(conv);
    ensureParticipant(conv, req.user);
    const result = await db_1.db.query(`SELECT m.*, u.name AS sender_name, u.role AS sender_role,
            u.profile_image_url AS sender_profile_image_url
     FROM messages m JOIN users u ON u.id = m.sender_id
     WHERE m.conversation_id = $1
     ORDER BY m.created_at ASC`, [conversationId]);
    await db_1.db.query(`INSERT INTO conversation_reads (user_id, conversation_id, last_read_at)
     VALUES ($1, $2, NOW())
     ON CONFLICT (user_id, conversation_id) DO UPDATE SET last_read_at = EXCLUDED.last_read_at`, [req.user.id, conversationId]);
    res.json(result.rows);
}
async function startAdminLandlordConversation(req, res) {
    const body = zod_1.z.object({ landlordId: zod_1.z.coerce.number().int().positive() }).parse(req.body);
    if (req.user?.role !== "admin")
        throw new http_1.HttpError(403, "Forbidden");
    const landlord = await db_1.db.query("SELECT id FROM users WHERE id = $1 AND role = 'landlord'", [body.landlordId]);
    if (!landlord.rowCount)
        throw new http_1.HttpError(404, "Landlord not found");
    const existing = await db_1.db.query("SELECT id FROM conversations WHERE landlord_id = $1 AND admin_landlord = true LIMIT 1", [body.landlordId]);
    if (existing.rowCount) {
        res.json({ conversationId: existing.rows[0].id });
        return;
    }
    const created = await db_1.db.query("INSERT INTO conversations (tenant_id, landlord_id, property_id, admin_landlord) VALUES (NULL, $1, NULL, true) RETURNING id", [body.landlordId]);
