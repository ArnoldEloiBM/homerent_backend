 ...
 */
async function ensureRentalStatusEnumValues() {
    const typ = await db_1.db.query(`SELECT oid FROM pg_type WHERE typname = 'RentalStatus'`);
    if (!typ.rowCount)
        return;
    const oid = typ.rows[0].oid;
    for (const label of ["pending", "rejected"]) {
        const exists = await db_1.db.query(`SELECT 1 FROM pg_enum WHERE enumtypid = $1 AND enumlabel = $2`, [oid, label]);
        if (exists.rowCount)
            continue;
        try {
            await db_1.db.query(`ALTER TYPE "RentalStatus" ADD VALUE '${label}'`);
        }
        catch (err) {
            const code = err?.code;
            if (code !== "42710")
...
async function main() {
  ...
  `);
    const existing = await db_1.db.query("SELECT id FROM users WHERE email = $1", ["eloibuyange@gmail.com"]);
    if (!existing.rowCount) {
        const password = await (0, security_1.hashPassword)("Admin@123");
        await db_1.db.query("INSERT INTO users (name, email, password, role, is_verified) VALUES ($1,$2,$3,'admin', true)", ["Arnold Buyange", "eloibuyange@gmail.com", password]);
    }
