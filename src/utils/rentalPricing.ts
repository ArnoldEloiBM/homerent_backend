 ...
 */
export function billingMonthsBetween(start: Date, end: Date): number {
  const startNorm = new Date(start.getFullYear(), start.getMonth(), start.getDate());
  const endNorm = new Date(end.getFullYear(), end.getMonth(), end.getDate());
  if (endNorm.getTime() <= startNorm.getTime()) return 0;
...
export function rentalTotalFromMonthlyPrice(
  monthlyPrice: number,
  startDateStr: string,
  endDateStr: string
): number {
